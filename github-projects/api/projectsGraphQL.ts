...
export const gqlGetProject = async (octokit: Octokit, { projectNumber }: { projectNumber: number }) => {
  const query = `query{
...
export const gqlGetIssuesForProject = async (
  octokit: Octokit,
  { projectNumber, findIssueNumbers = [] }: { projectNumber: number; findIssueNumbers?: number[] },
  limitOptions?: {
    issueCount?: number;
